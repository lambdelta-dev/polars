...
## JSON reading changes

### Set ndjson `infer_schema_files` default value to `10`

Multi-file NDJSON scans now inspect the first 10 files (instead of only the first file) when inferring
the schema. Previously the schema was always sourced from the first file, which could raise
`ComputeError` when a later file contained a non-null value for a column that was all-null in the
first file.

The new `infer_schema_files` parameter on `read_ndjson`/`scan_ndjson` controls how many files are used.
It mirrors the parameter of the same name on `read_csv`/`scan_csv`.
...
```pycon
>>> pl.scan_ndjson([b'{"a": null}', b'{"a": [1, 2]}']).collect()
Traceback (most recent call last):
...
ComputeError: got non-null value for NULL-typed column: [1, 2]
```

After:

```pycon
>>> pl.scan_ndjson([b'{"a": null}', b'{"a": [1, 2]}']).collect_schema()
Schema({'a': List(Int64)})
```

Use `infer_schema_files=1` to restore the old behavior of only using the first file.
